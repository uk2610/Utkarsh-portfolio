...
function ArticleTextItem({ itemWrapper }) {
    ...
    const linkedinDmUrl = `https://www.linkedin.com/in/Uk2610/`
    const isAboutText = true

    const _onConnectClicked = () => {
        window.open(linkedinDmUrl, `_blank`, `noreferrer`)
    }
    ...
    return (
        <div className={`article-text-item ${positioningClass}`}>
            ...
            {isAboutText && (
                <div className={`article-text-cta-wrapper`}>
                    <StandardButton label={`Let's Connect`}
                                    onClick={_onConnectClicked}
                                    faIcon={`fa-brands fa-linkedin-in`}
                                    displayIconAsSuffix={true}
                                    className={`article-text-cta-btn`}
                                    variant={`contrast`}/>
                </div>
            )}
        </div>
